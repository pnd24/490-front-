...
function populateExams(response){
	...
	for (var i in examDB){
		...
		switch(examDB[i]['status']){
			case 'Assigned':
				edit_td.innerHTML = '<div><input class="tiny-button" type="button" value="Take" onClick="takeExam('+i+')"></div>';
				break;
			case 'Submitted':
				edit_td.innerHTML = '<div><p>Wait for Results</p></div>';
				break;
			case 'Released':
				edit_td.innerHTML = '<div><input class="tiny-button" type="button" value="Results" onClick="reviewExam('+i+')"></div>';
				break;
			default:
...
function takeExam(examID)
{
	...
	goTo('https://web.njit.edu/~pnd23/student/take-exam.html')
}
function reviewExam(examID){
	window.localStorage.setItem('examName', examDB[examID]['examName']);
	window.localStorage.setItem("studentName", window.localStorage.getItem('username'));
	goTo('https://web.njit.edu/~pnd23/student/reviewExam.html')
}
